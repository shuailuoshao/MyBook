...
function validateBooksData(books) {
  ...
  const validStatuses = ['未开始', '阅读中', '已读完'];
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    if (!book.id || typeof book.id !== 'string') {
      return { valid: false, error: `第${i + 1}本书缺少有效的ID` };
    }
    if (!book.title || typeof book.title !== 'string') {
      return { valid: false, error: `第${i + 1}本书缺少有效的标题` };
    }

    // 验证状态
    if (book.status && !validStatuses.includes(book.status)) {
      return { valid: false, error: `第${i + 1}本书的状态无效` };
    }

    // 防止原型污染 - 删除危险属性
    delete book.__proto__;
    delete book.constructor;
  }
...
async function atomicWriteFile(filePath, data) {
  ...
  try {
    ...
    try {
      await atomicWriteFile(BOOKS_FILE, JSON.stringify(books, null, 2));
      return { success: true };
    } finally {
      releaseWriteLock();
    }
...
ipcMain.handle('load-books', async () => {
  try {
    ...
    try {
      ...
      console.error('JSON解析失败，文件可能已损坏:', parseError);
      // 文件损坏时返回空数组并尝试修复
      await fs.writeFile(BOOKS_FILE, JSON.stringify([], null, 2), 'utf-8');
      return [];
    }
...
ipcMain.handle('delete-book', async (event, bookId) => {
  try {
    ...
    // 保存更新后的数据
    await fs.writeFile(BOOKS_FILE, JSON.stringify(updatedBooks, null, 2), 'utf-8');
    return { success: true };
  } catch (error) {
...
ipcMain.handle('import-data', async (event, { filePath }) => {
  try {
    ...
    if (stats.size > 5 * 1024 * 1024) {
      return { success: false, error: '文件过大（最大 5MB）' };
    }
